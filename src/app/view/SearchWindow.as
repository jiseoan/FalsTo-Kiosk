package app.view 
{
	...
	public class SearchWindow extends Sprite 
	{
		...
		public function SearchWindow() 
		{
			...
			_closeBtn = new Button([new ImageBox("search_close"), new ImageBox("search_close_press") ]);
			_closeBtn.alpha = 0;
		...
		{
			Model.instance.searchBrands(_searchForm.keyword);
		}
		...
		public function close(sec:Number=0.3):void
		{
			...
			TweenMax.to(_tab, 0.3, { autoAlpha:0} );
			TweenMax.to(_keypad, 0.3, { autoAlpha:0 } );
			TweenMax.to(_bg, 0.3, { autoAlpha:0} );
			TweenMax.to(_closeBtn, 0.3, { autoAlpha:0} );
