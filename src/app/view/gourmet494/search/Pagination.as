package app.view.gourmet494.search 
{
	...
	public class Pagination extends Sprite 
	{
		...
		public function Pagination(totalPages:int=1) 
		{
			...
			_nexBtn = new Button([new ImageBox("gourmet/btn_next"), new ImageBox("gourmet/btn_next_press")]);
			_nexBtn.addEventListener(Button.CLICK, onClickNextBtn, false, 0, true);
			addChild(_nexBtn);
			
			for (var i:int = 1; i <=5; i++) 
			{
				var pageBtn:Button = new Button([new ImageBox("gourmet/btn_page_disable"), new ImageBox("gourmet/btn_page_focus"), new ImageBox("gourmet/btn_page_disable"), new ImageBox("gourmet/btn_page_focus")]);
				pageBtn.x = 61 + (i-1) * 61;
				pageBtn.id = i;
		...
		public function update(curPage:int, totalPages:int):void
		{
			...
			for (var i:int = 1; i <= 5; i++) 
			{
				...
				{
					TextFormatter.setTextFormat(pageBtn.tf, 0x000000, 24, MyriadProBold, "center", false, 0, true);
